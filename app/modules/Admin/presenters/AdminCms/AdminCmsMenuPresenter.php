...
//		$this->template->tree = $tree;
	...
	public function menuItemEditFormSubmitted(TranslatedForm $form, $values) {
		try {
			$item = MenuItem::get($values['id']);
			$item->menu = Menu::get($values['menuid'])->getEntity();
			$item->active = $values['active'];
			$item->text = $values['text'];
			$item->parent = !empty($values['parent']) ? MenuItem::get($values['parent'])->getEntity() : NULL;
