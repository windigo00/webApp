...
class UserManager extends Nette\Object implements Nette\Security\IAuthenticator, \Nette\Security\IAuthorizator
{
	...
	{
		$database = \App\Management\EntityManager::get();
		$this->database = $database->getRepository('App\Model\User');
	}
	...
	public function authenticate(array $credentials)
	{
		...
		} elseif (Passwords::needsRehash($row->needRehash)) {
			$row->setPassword(Passwords::hash($password));
			$row->persist();
		}
		return new Nette\Security\Identity($row->getId(), $row->getRole(), $row);
	}
